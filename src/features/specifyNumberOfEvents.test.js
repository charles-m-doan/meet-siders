...
defineFeature(feature, (test) => {
   ...
   test('When user hasn’t specified a number, 1 is the default number.', ({ given, when, then }) => {
      ...
      when('The list is displayed to the user with default settings', () => {});
   ...
   //Scenario #2:
   test('User can change the number of events they want to see.', async ({ given, when, then }) => {
      given('A list of events was loaded', async () => {
         render(<App />);
         await waitFor(() => expect(screen.getByTestId('event-list')).toBeInTheDocument()); // Wait for events to be loaded
      });
      ...
      when('The user specifies how many events they want displayed at one time', async () => {
         ...
         await waitFor(() => expect(input.value).toBe('1')); // Wait for input value to change
      });
